...
def get_cwt(t, sig, fmin, fmax, dj=0.0025, normalization=False):
    dt = np.diff(t)[0]
    if sig.ndim == 1:
        sig_pt = sig.reshape(1,1,-1)
    sig_pt = torch.tensor(sig_pt, dtype=torch.float32)
    pycwt = CWT(dt=dt, dj=dj, fmin=fmin, fmax=fmax)
    pycwt.magnitude = pycwt(sig_pt).squeeze().numpy()
    if normalization:
        pycwt.magnitude = zscore(pycwt.magnitude, axis=0)
    pycwt.t = t
    pycwt.sig = sig
    pycwt.freq = pycwt._freq
    return pycwt

def plot_cwt(sig_cwt, cmap='jet'):
    fig, axes = plt.subplots(nrows=2, figsize=(10, 10))
    axes[0].plot(sig_cwt.t, sig_cwt.sig)
    axes[1].pcolormesh(sig_cwt.t, sig_cwt.freq,
                       sig_cwt.magnitude, cmap=cmap, shading='gouraud')
...
class CWT(nn.Module):
    ...
    def compute_optimal_scales(self):
        ...
        frequencies = np.array(
            [1 / self.wavelet.fourier_period(s) for s in scales])
        if self.fmin:
